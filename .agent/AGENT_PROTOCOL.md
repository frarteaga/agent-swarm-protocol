...
**[PROTO-AUTHORITY-01] Role boundary.** Specifier owns **WHAT** externally visible behavior is required; architect owns **HOW** responsibilities/dependencies/trust boundaries are structured; developer owns implementation/remediation; reviewer owns general correctness/maintainability review; security owns adversarial security assessment and security-specific verification; QA owns final acceptance validation. No role silently absorbs another role's authority.

**[PROTO-SPEC-BLOCK-01] Spec block.** Specifier MAY inspect implementation/PRs and directly block contradiction of accepted requirement/criterion/agreed behavior/human-approved specification using `MSG-SPEC-BLOCK-01`. While unresolved: PR MUST NOT advance to QA; task MUST NOT be done; PR SHOULD NOT be merged; reviewer approval/architecture/security preference do not override; human may override.

**[PROTO-SPEC-CLEAR-01] Spec clear.** After fix, developer returns to specifier. When conforming, specifier uses `MSG-SPEC-CLEAR-01`.

## Review, security, QA, completion, regression

**[PROTO-REVIEW-01] Review outcomes.** Approved -> next valid gate (architect hardening by default; security before QA when required); changes -> developer with actionable findings; architecture question -> architect with one concrete block. Reviewer does not own substantial architecture or security signoff unless explicitly granted.

**[PROTO-SECURITY-GATE-01] Risk-based security gate.** Security is required when work changes authentication/authorization; secrets/credentials; cryptography; network exposure; privileged execution or shell/process spawning; file-system/sandbox boundaries; handling of external or untrusted input/content; agent/tool permissions; CI/CD or GitHub Actions permissions; dependency/supply-chain trust; persistent sensitive data; or infrastructure/security configuration. A human or architect MAY also require it for any task. Architect (or human) MUST record `SECURITY_GATE: REQUIRED|NOT_REQUIRED` when security selection must be made. If any listed risk criterion applies, the decision MUST be `SECURITY_GATE: REQUIRED`; `NOT_REQUIRED` is valid only when all listed criteria were evaluated and none apply, with a short rationale. Only an explicit human override recorded durably under `PROTO-HUMAN-01` may waive a gate that would otherwise be required. If a listed criterion plausibly applies and no decision exists, developer/reviewer/QA MUST fail closed and route to architect rather than bypass Security. Unrelated low-risk work does not require the gate.

**[PROTO-SECURITY-01] Security outcomes.** Security uses `MSG-SECURITY-RESULT-01` with one exact result: `SECURITY PASS`, `SECURITY CHANGES REQUIRED`, `SECURITY ARCHITECTURE BLOCK`, or `SECURITY SPEC BLOCK`. PASS requires current reproducible evidence and re-verification of remediations when findings existed. Changes route to developer; structural/trust-boundary blocks to architect; behavior/requirement conflicts to specifier. A required gate with any unresolved blocking result MUST NOT advance to QA/done unless explicitly human-overridden.

**[PROTO-COMPLETE-01] QA outcomes.** PASS uses `MSG-COMPLETE-01`; when completion is valid normalize done/remove owner as appropriate. FAIL hands to developer with reproducible evidence. When `SECURITY_GATE: REQUIRED`, QA MUST confirm a current `SECURITY PASS` for the implementation revision before completion.
...
**[PROTO-FLOW-01] Flow.** Default `specifier -> architect -> developer -> reviewer -> architect(hardening) -> qa -> done`; Security is a risk-based gate inserted before QA when required. Common valid transitions: `specifier->architect|developer`, `architect->developer|specifier|reviewer|security|qa`, `developer->reviewer|architect|specifier`, `reviewer->developer|architect|security|qa`, `security->developer|architect|specifier|qa`, `qa->developer|reviewer|architect|security|specifier|done`. In particular `reviewer->security` is valid when architect hardening is skipped/already current. Skip unnecessary roles when appropriate; humans may override.
...
**[PROTO-COMMS-01] Communication.** SHOULD be concise, factual, actionable, durable, scannable; avoid repeating GitHub context. Communicate on claim, handoff, block, decision, important finding, stale recovery, security result, completion. Human-facing progress SHOULD lead with semantic outcome/phase/blocker/next step; SHAs/run/CI IDs are supporting evidence unless requested or needed. Exact durable engineering evidence remains required where specified.
