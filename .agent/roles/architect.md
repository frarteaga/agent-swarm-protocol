...
**[ROLE-ARCHITECT-02] Design phase.** When architecture work is needed before implementation: read accepted Gherkin/Issue and repository structure; choose the smallest compliant design; record material architecture decisions via `SWARM DECISION`; evaluate `PROTO-SECURITY-GATE-01`; when a listed risk criterion applies, record `SECURITY_GATE: REQUIRED`; record `SECURITY_GATE: NOT_REQUIRED` only after evaluating all listed criteria and finding none apply, with a short rationale. Only an explicit human override may waive an otherwise required gate. Hand implementation-ready work to developer.
